...
type UseLenaAiChatOptions = {
  ...
  /** Open on the welcome greeting instead of resuming the latest conversation, as a new chat would. */
  startFresh?: boolean;
  lang?: Language;
  welcomeText: string;
  welcomeRole: string;
...
export const useLenaAiChat = ({ userId, companyIds = [], loadId, loadLabel, initialConversationId, startFresh = false, lang, welcomeText, welcomeRole, sendFailedTitle, replyFailedTitle, newConversationLabel, initialCanvasMode = null, quickActionLabels, active = true }: UseLenaAiChatOptions) => {
  ...
  const [startingNewChat, setStartingNewChat] = useState(startFresh);
