...
export const loadGoogleIdentityScript = (lang?: string): Promise<void> => {
  ...
  if (window.google?.accounts?.id) {
    ...
    // (from an earlier page load, or one still hanging around from before this `hl` wiring
    // existed) can't just be re-localized in place — tear it down and fetch it again.
