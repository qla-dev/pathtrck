...
export const LoadOffersPanel = ({
  ...
  const [providerOffer, setProviderOffer] = useState<Record<string, unknown> | null>(null);
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {visibleOffers.map((offer) => {
            ...
            const decided = ['accepted', 'not_selected', 'rejected', 'withdrawn', 'expired', 'cancelled'].includes(status);
              ...
              <article key={offerId} className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50/60 p-4 dark:border-slate-800 dark:bg-slate-950/40 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-stretch">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        {role === 'user' ? (
                          <button type="button" onClick={() => setProviderOffer(offer)} className="max-w-full cursor-pointer truncate text-left text-lg font-black text-slate-900 underline-offset-4 transition hover:text-primary hover:underline dark:text-white">
                            {company?.name || creator?.name || u('Independent offer', 'Independent offer')}
                          </button>
                        ) : (
                ...
                <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                  {role === 'user' && (
                    <button
                      type="button"
                      onClick={() => setProviderOffer(offer)}
                      className="flex h-10 w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-slate-200 text-sm font-bold text-slate-700 transition hover:border-primary hover:text-primary dark:border-slate-700 dark:text-slate-200"
                    >
      ...
      <ProviderDetailsModal
        open={Boolean(providerOffer)}
        lang={lang}
        offer={providerOffer}
        onClose={() => setProviderOffer(null)}
      />
