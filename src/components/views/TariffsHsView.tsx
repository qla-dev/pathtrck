    ...
    <div className="w-full min-w-0 space-y-6">
      ...
      <section className="space-y-3">
        ...
        <div className="grid auto-cols-[minmax(220px,1fr)] grid-flow-col gap-3 overflow-x-auto pb-2">
          <button type="button" onClick={() => setSelectedSection('')} className={cn('min-h-24 rounded-2xl border p-4 text-left transition-all', !selectedSection ? 'border-primary bg-primary text-white shadow-lg shadow-primary/15' : 'border-slate-200 bg-white text-slate-700 hover:border-primary/40 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200')}>
          ...
          {categoriesLoading
            ...
            : categories.map((category, index) => (
              <button key={category.id} type="button" onClick={() => setSelectedSection(category.id)} className={cn('min-h-24 rounded-2xl border p-4 text-left transition-all', selectedSection === category.id ? 'border-primary bg-primary text-white shadow-lg shadow-primary/15' : CATEGORY_TONES[index % CATEGORY_TONES.length])}>
      ...
      <TariffTable lang={lang} section={selectedSection} refreshKey={refreshToken} />
    </div>
  );
};
