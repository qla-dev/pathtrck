...
export const AdminCompaniesView = ({ lang: _lang, onOpenEmailStudio }: { lang: Language; onOpenEmailStudio?: () => void }) => {
  ...
  const save = async () => { setSubmitting(true); setError(''); try { await api.companies.onboard(form); await companies.refresh(); setOpen(false); setForm(initial); } catch (caught) { const validation = caught instanceof ApiError ? Object.values(caught.errors).flat()[0] : null; setError(validation || (caught instanceof Error ? caught.message : 'Company could not be created.')); } finally { setSubmitting(false); } };
  return <>
    <div className="space-y-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-500/10 text-violet-500"><Building2 className="h-6 w-6" /></div><div><p className="text-xs font-black uppercase tracking-[0.18em] text-primary">Superadmin registry</p><h1 className="text-2xl font-black dark:text-white">Logistics Companies</h1></div></div><div className="flex gap-2"><Button variant="outline" onClick={onOpenEmailStudio}><Mail className="mr-2 h-4 w-4" />Email</Button><Button onClick={() => setOpen(true)}>Add company</Button></div></div><p className="mt-4 text-sm text-slate-500">Inspect every company, owner, subscription, fleet and verification state.</p></section>
