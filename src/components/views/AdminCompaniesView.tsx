...
import { Building2, Clock3, Crown, Eye, Mail, Search } from 'lucide-react';
...
export const AdminCompaniesView = ({ lang: _lang, onOpenEmailStudio }: { lang: Language; onOpenEmailStudio?: () => void }) => {
  ...
  return <>
    <div className="space-y-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-500/10 text-violet-500"><Building2 className="h-6 w-6" /></div><div><h1 className="text-2xl font-black dark:text-white">Logistics Companies</h1><p className="mt-1 text-sm text-slate-500">Inspect every logistics company, owner, subscription, fleet and verification state.</p></div></div><div className="flex gap-2"><Button variant="outline" onClick={onOpenEmailStudio}><Mail className="mr-2 h-4 w-4" />Email</Button><Button onClick={() => setOpen(true)}>Add company</Button></div></div></section>
      <div className="grid gap-3 sm:grid-cols-3"><Card className="shadow-none" contentClassName="flex items-center justify-between gap-3 px-5 py-3"><div><p className="text-xs uppercase text-slate-500">Total companies</p><p className="mt-1 text-2xl font-black dark:text-white">{companies.total}</p></div><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet-500/10 text-violet-500"><Building2 className="h-6 w-6" /></div></Card><Card className="shadow-none" contentClassName="flex items-center justify-between gap-3 px-5 py-3"><div><p className="text-xs uppercase text-slate-500">Pending verification</p><p className="mt-1 text-2xl font-black text-amber-500">{companies.items.filter((row) => row.status === 'pending').length}</p></div><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-amber-500/10 text-amber-500"><Clock3 className="h-6 w-6" /></div></Card><Card className="shadow-none" contentClassName="flex items-center justify-between gap-3 px-5 py-3"><div><p className="text-xs uppercase text-slate-500">Enterprise accounts</p><p className="mt-1 text-2xl font-black text-violet-500">{companies.items.filter((row) => row.plan === 'enterprise').length}</p></div><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet-500/10 text-violet-500"><Crown className="h-6 w-6" /></div></Card></div>
      <Card className="shadow-none"><div className="relative max-w-md"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search companies..." className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[900px] text-left"><thead><tr className="border-b border-slate-200 text-xs uppercase text-slate-500 dark:border-slate-800"><th className="p-3">Company</th><th className="p-3">Owner</th><th className="p-3">Plan</th><th className="p-3">Fleet</th><th className="p-3">Members</th><th className="p-3">Status</th><th className="p-3"></th></tr></thead><tbody>{visible.map((row) => <tr key={String(row.id)} className="border-b border-slate-100 dark:border-slate-800"><td className="p-3"><p className="font-bold dark:text-white">{String(row.name)}</p><p className="text-xs text-slate-500">{String(row.country_code || '—')} · {String(row.email || '—')}</p></td><td className="p-3 text-sm">{String(((row.owner || {}) as Record<string, unknown>).name || '—')}</td><td className="p-3 font-bold text-violet-500">{String(row.plan || '—')}</td><td className="p-3">{Array.isArray(row.vehicles) ? row.vehicles.length : 0}</td><td className="p-3">{Array.isArray(row.users) ? row.users.length : 0}</td><td className="p-3"><span className="rounded-full bg-primary/10 px-2 py-1 text-xs font-bold text-primary">{String(row.status || '—')}</span></td><td className="p-3"><button onClick={() => setSelected(row)} className="rounded-lg bg-slate-100 p-2 dark:bg-slate-800"><Eye className="h-4 w-4" /></button></td></tr>)}</tbody></table></div></Card>
