  ...
  return <>
    <div className="space-y-6">
      <PageHeader
        ...
        stats={[
          { label: 'Total companies', value: companies.total, icon: Building2, tone: 'bg-violet-500/10 text-violet-500' },
          { label: 'Pending verification', value: companies.items.filter((row) => row.status === 'pending').length, icon: Clock3, tone: 'bg-amber-500/10 text-amber-500' },
          { label: 'Enterprise accounts', value: companies.items.filter((row) => row.plan === 'enterprise').length, icon: Crown, tone: 'bg-violet-500/10 text-violet-500' },
        ]}
      />
