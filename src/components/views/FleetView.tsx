...
export const FleetView = ({ lang }: { lang: Language }) => {
  ...
  const vehicles = [
	    ...
	    { id: 'V2', model: 'Volkswagen Crafter', plate: 'DE-992-AB', status: 'Maintenance', fuel: '20%', fuelType: 'Diesel', trailer: 'Yes (1)', tailLift: 'No', nextService: lang === 'bs' ? 'Sutra' : lang === 'de' ? 'Morgen' : 'Tomorrow', location: [43.8463, 18.4031] },
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: lang === 'bs' ? 'Ukupno vozila' : lang === 'de' ? 'Fahrzeuge gesamt' : 'Total Vehicles', value: '12', icon: Truck, color: 'text-blue-600', bg: 'bg-blue-100' },
          { label: lang === 'bs' ? 'Aktivno sada' : lang === 'de' ? 'Jetzt aktiv' : 'Active Now', value: '8', icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-100' },
          { label: lang === 'bs' ? 'Na održavanju' : lang === 'de' ? 'In Wartung' : 'In Maintenance', value: '2', icon: Settings, color: 'text-amber-600', bg: 'bg-amber-100' },
          { label: lang === 'bs' ? 'Prosj. efikasnost' : lang === 'de' ? 'Ø Effizienz' : 'Avg Efficiency', value: '89%', icon: BarChart3, color: 'text-purple-600', bg: 'bg-purple-100' },
        ].map((stat, i) => (
          ...
          <div className="space-y-4">
            {vehicles.filter(v => v.status === 'Maintenance' || v.nextService === 'Tomorrow').map((v, i) => (
              <div key={i} className="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{lang === 'bs' ? 'Servis dospijeva:' : lang === 'de' ? 'Service fällig:' : 'Service due:'} <span className="font-bold text-amber-600">{v.nextService}</span></p>
                  <Button size="sm" variant="outline" className="mt-2 h-7 text-[10px] rounded-full">{lang === 'bs' ? 'Zakaži sada' : lang === 'de' ? 'Jetzt planen' : 'Schedule Now'}</Button>
                </div>
              </div>
            ))}
            <div className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium dark:text-white">{lang === 'bs' ? 'Sva ostala vozila su sigurna' : lang === 'de' ? 'Alle anderen Fahrzeuge sicher' : 'All other vehicles safe'}</span>
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left border-b border-slate-100 dark:border-slate-800">
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Vozilo' : lang === 'de' ? 'Fahrzeug' : 'Vehicle'}</th>
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Registarska oznaka' : lang === 'de' ? 'Kennzeichen' : 'License Plate'}</th>
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Tip goriva' : lang === 'de' ? 'Kraftstoffart' : 'Fuel Type'}</th>
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Prikolica' : lang === 'de' ? 'Anhänger' : 'Trailer'}</th>
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Rampa' : lang === 'de' ? 'Hebebühne' : 'Tail Lift'}</th>
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Status' : lang === 'de' ? 'Status' : 'Status'}</th>
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Nivo goriva' : lang === 'de' ? 'Kraftstoffstand' : 'Fuel Level'}</th>
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Sljedeći servis' : lang === 'de' ? 'Nächster Service' : 'Next Service'}</th>
                <th className="p-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest">{lang === 'bs' ? 'Akcije' : lang === 'de' ? 'Aktionen' : 'Actions'}</th>
              </tr>
            </thead>
            <tbody>
              {vehicles.map(v => (
