...
export const SettingsView = ({
  ...
  useEffect(() => { void api.auth.me().then((value) => { setUser(value); setProfileForm({ name: value.name || '', email: value.email || '', phone: String((value as ApiUser & { phone?: string }).phone || '') }); }); }, []);
  const saveProfile = async () => { const updated = await api.auth.updateProfile(profileForm); setUser(updated); setSaveStatus(u('legacy.settings.saved', 'Saved')); };
        ...
        <Card className="xl:col-span-4">
          ...
          <div className="space-y-3">
            <label className="block">
              ...
              <input className="mt-1.5 w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 text-sm dark:text-white" value={profileForm.name} onChange={(event) => setProfileForm((current) => ({ ...current, name: event.target.value }))} />
            </label>
            <label className="block">
              <span className="text-xs font-semibold text-slate-500">{u('legacy.settings.email', 'Email')}</span>
              <input className="mt-1.5 w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 text-sm dark:text-white" value={profileForm.email} onChange={(event) => setProfileForm((current) => ({ ...current, email: event.target.value }))} />
            </label>
            <label className="block">
              <span className="text-xs font-semibold text-slate-500">{u('legacy.settings.phone', 'Phone')}</span>
              <input className="mt-1.5 w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 text-sm dark:text-white" value={profileForm.phone} onChange={(event) => setProfileForm((current) => ({ ...current, phone: event.target.value }))} />
            </label>
          </div>
        </Card>
        ...
        <Card className="xl:col-span-4">
          ...
          <div className="space-y-3">
            {isDriver ? (
              ...
              <>
                <InfoRow label={u('legacy.settings.company', 'Company')} value={String((user?.companies?.[0] as Record<string, unknown> | undefined)?.name || '—')} />
                <InfoRow label={u('legacy.settings.vatId', 'VAT ID')} value={String((user?.companies?.[0] as Record<string, unknown> | undefined)?.tax_number || '—')} />
