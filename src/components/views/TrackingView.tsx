...
const TrackingCardsSkeleton = ({ layout }: { layout: TrackingLayoutMode }) => (
  <div className={cn('mt-4 animate-pulse gap-4', layout === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3' : 'flex flex-col')}>
...
export const TrackingView = ({ lang, role, userId, companyIds = [], onLayoutModeChange, requestedLayout, requestedLayoutNonce }: TrackingViewProps) => {
  ...
  // Capacity is read per vehicle, not as one number for the whole fleet: a truck that is full says
  // nothing about the one standing empty next to it. Loads with no vehicle yet get their own card.
  const fleetCapacity = useMemo(() => {
    const byVehicle = new Map<number, { weightKg: number; loads: number }>();
    let unassignedWeightKg = 0;
    let unassignedLoads = 0;
    loadCapacity.activeLoads.forEach((load) => {
      const vehicleId = Number(load.vehicle_id || 0);
      const weightKg = Number(load.weight_kg || 0);
      if (!vehicleId) {
        unassignedWeightKg += weightKg;
        unassignedLoads += 1;
        return;
      }
      const current = byVehicle.get(vehicleId) || { weightKg: 0, loads: 0 };
      byVehicle.set(vehicleId, { weightKg: current.weightKg + weightKg, loads: current.loads + 1 });
    });

    const card = (key: string, title: string, subtitle: string, capacityKg: number, weightKg: number, loads: number) => {
      const usedPercentage = capacityKg > 0 ? Math.min(100, Math.round((weightKg / capacityKg) * 100)) : 0;
      return {
        key, title, subtitle, capacityKg, weightKg, loads,
        usedPercentage,
        remainingPercentage: Math.max(0, 100 - usedPercentage),
      };
    };

    const cards = fleetResult.items.map((vehicle) => {
      const stats = byVehicle.get(Number(vehicle.id)) || { weightKg: 0, loads: 0 };
      const subtitle = [vehicle.make, vehicle.model].filter(Boolean).join(' ') || String(vehicle.vehicle_type || '');
      return card(
        `vehicle-${vehicle.id}`,
        String(vehicle.registration_number || `#${vehicle.id}`),
        subtitle,
        Number(vehicle.capacity_kg || 0) || TRUCK_CAPACITY_KG,
        stats.weightKg,
        stats.loads,
      );
    });

    if (unassignedLoads > 0) {
      cards.push(card(
        'unassigned',
        u('tracking.unassignedFleet', 'No vehicle assigned'),
        u('tracking.unassignedFleetHint', 'Waiting for a vehicle'),
        TRUCK_CAPACITY_KG,
        unassignedWeightKg,
        unassignedLoads,
      ));
        ...
        <div className={cn('overflow-visible rounded-2xl border border-slate-200 dark:border-slate-800', layout === 'map' ? 'pointer-events-auto border-white/60 bg-white/25 shadow-md shadow-slate-900/10 backdrop-blur-md dark:border-white/10 dark:bg-slate-900/25' : 'bg-white dark:bg-slate-900')}>
            <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">
                <div className="flex h-9 min-w-0 max-w-full items-center gap-2 overflow-x-auto rounded-xl bg-slate-50 px-3 dark:bg-slate-950/60">
            ...
            >
              <div className="mb-2 flex items-center justify-between gap-3">
                <span className="flex min-w-0 items-center gap-2">
                  <span className="min-w-0 truncate font-mono text-sm font-bold text-primary">{pkg.trackingNumber}</span>
                  {pendingActionsFor(pkg) > 0 && (
                    <span
                      title={u('shipmentDetails.actionsNeeded', 'Actions needed')}
                      className="inline-flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-rose-500 px-1.5 text-[10px] font-black text-white"
                    >
                      {pendingActionsFor(pkg)}
                    </span>
                  )}
                </span>
