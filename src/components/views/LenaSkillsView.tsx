...
import { BookOpen, Braces, BrainCircuit, ChevronDown, CornerDownRight, ExternalLink, FileText, Layers3, Network, RefreshCw, Search, Sparkles } from 'lucide-react';
...
// A mode folder's AGENT.md is the skill; the files beside it (skills/*.md, hs-detection.md) are its subskills.
...
export const LenaSkillsView = ({ lang }: { lang: Language }) => {
  ...
  const matches = (row: LenaSkillRow) => `${row.name} ${row.description} ${row.id} ${row.modes.join(' ')} ${row.content}`.toLocaleLowerCase().includes(query);
  ...
  const tree = useMemo(() => {
    ...
    for (const row of rows) {
      const group = byFolder.get(row.folder) ?? { folder: row.folder, subskills: [] };
      if (row.kind === 'instructions') group.skill = row; else group.subskills.push(row);
      byFolder.set(row.folder, group);
    }
    return [...byFolder.values()].flatMap(group => {
      const skillMatches = !!group.skill && matches(group.skill);
      ...
      <td className="min-w-60 p-4 align-top">
        <div className={cn('flex items-start gap-2', nested && 'pl-8')}>
          ...
          <div className="min-w-0">
            <button type="button" aria-label={`${t.open}: ${row.name}`} className="cursor-pointer text-left font-bold text-slate-800 hover:text-primary dark:text-slate-100">{row.name}</button>
  ...
  return <div className="w-full min-w-0 space-y-5">
    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,1fr)]">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">{!loading && (filter === 'all'
            ? tree.map(group => {
              ...
              return <Fragment key={group.folder}>
                {group.skill && renderRow(group.skill, { group, open })}
                {(!group.skill || open) && group.subskills.map(row => renderRow(row, { nested: !!group.skill }))}
              </Fragment>;
            })
      ...
      <aside className="min-w-0 self-start rounded-2xl border border-violet-200 bg-white dark:border-violet-900 dark:bg-slate-900 xl:sticky xl:top-4">
        ...
        {selected && !loading ? <div className="space-y-3 p-4"><h3 className="font-bold text-slate-900 dark:text-white">{selected.name}</h3><div className="flex flex-wrap gap-1"><span className="rounded-md bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{selected.kind === 'instructions' ? t.skill : t.subskill}</span>{selected.modes.map(mode => <span key={mode} className="rounded-md bg-violet-50 px-2 py-1 text-[10px] text-violet-600 dark:bg-violet-950 dark:text-violet-300">{mode}</span>)}</div>{selected.scanners && <p className="text-xs text-emerald-600">{t.scanner}</p>}
          ...
          {tab === 'overview'
            ...
            : (selected.sources ?? []).length
              ...
              ? <ul className="max-h-[65vh] divide-y divide-slate-100 overflow-y-auto rounded-xl border border-slate-100 dark:divide-slate-800 dark:border-slate-800">{(selected.sources ?? []).map(source => <li key={source.id}><a href={`${API_BASE_URL}/legal-sources/${encodeURIComponent(source.id)}`} target="_blank" rel="noreferrer" title={source.title} className="flex min-w-0 cursor-pointer items-center gap-2 px-3 py-2.5 text-xs font-semibold text-primary hover:bg-violet-50 hover:underline dark:hover:bg-violet-950/30"><FileText className="h-3.5 w-3.5 shrink-0" /><span className="min-w-0 flex-1 truncate">{source.file}</span><ExternalLink className="h-3.5 w-3.5 shrink-0" /></a></li>)}</ul>
