import { Activity, Boxes, Building2, Crown, PackageCheck, ReceiptText, ShieldCheck, Truck, Users } from 'lucide-react';
...
export const AdminOverviewView = ({ lang: _lang }: { lang: Language }) => {
  ...
  return <div className="space-y-6">
    ...
    {error ? <Card><InlineDataState loading={false} error={error} empty="" onRetry={() => sources.forEach((source) => void source.refresh())} /></Card> : <>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">{metrics.map((metric) => <Card key={metric.label} className="p-4"><div className="flex items-start justify-between"><div><p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">{metric.label}</p><p className="mt-2 text-2xl font-black text-slate-900 dark:text-white">{loading ? '—' : metric.value}</p></div><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${metric.tone}`}><metric.icon className="h-5 w-5" /></div></div></Card>)}</section>
