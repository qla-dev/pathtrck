...
export const AdminOverviewView = ({ lang: _lang }: { lang: Language }) => {
  ...
  return <div className="space-y-6">
    <section className="relative overflow-hidden rounded-3xl border border-sky-100 bg-gradient-to-br from-white via-sky-50 to-violet-100 p-6 text-slate-900 shadow-xl shadow-sky-900/10 dark:border-slate-800 dark:from-slate-950 dark:via-slate-950 dark:to-indigo-950 dark:text-white dark:shadow-black/30 md:p-8">
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-6"><div className="flex items-center gap-3"><div className="flex h-13 w-13 items-center justify-center rounded-2xl bg-gradient-to-br from-primary to-violet-500 text-white shadow-lg"><Crown className="h-7 w-7" /></div><div><h1 className="text-3xl font-black">Platform Command Center</h1><p className="mt-1 max-w-3xl text-sm text-slate-600 dark:text-slate-300">Live visibility across records stored in the Smartfreight database.</p></div></div><div className="rounded-2xl border border-emerald-300/70 bg-white/70 px-4 py-3 dark:border-emerald-400/20 dark:bg-emerald-400/10"><p className="flex items-center gap-2 text-xs font-black text-emerald-600 dark:text-emerald-400"><ShieldCheck className="h-4 w-4" /> DATABASE CONNECTED</p></div></div>
    ...
    {error ? <Card><InlineDataState loading={false} error={error} empty="" onRetry={() => sources.forEach((source) => void source.refresh())} /></Card> : <>
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">{metrics.map((metric) => <Card key={metric.label} className="shadow-none" contentClassName="flex items-center justify-between gap-3 px-5 py-3"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500">{metric.label}</p><p className="mt-1 text-2xl font-black text-slate-900 dark:text-white">{loading ? '—' : metric.value}</p></div><div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${metric.tone}`}><metric.icon className="h-6 w-6" /></div></Card>)}</section>
