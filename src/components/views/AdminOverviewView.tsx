import { Activity, Building2, Crown, PackageCheck, ReceiptText, ShieldCheck, Truck, Users } from 'lucide-react';
...
export const AdminOverviewView = ({ lang: _lang }: { lang: Language }) => {
  ...
  const sources = [companies, drivers, loads, vehicles, invoices, events];
  const loading = sources.some((source) => source.loading);
  const error = sources.find((source) => source.error)?.error;
  const activeLoads = loads.items.filter((load) => !['completed', 'cancelled'].includes(String(load.status).toLowerCase()));
  ...
  const countries = new Set(companies.items.map((company) => String(company.country_code || '')).filter(Boolean)).size;
  const metrics = [
    ...
    { label: 'Active loads', value: activeLoads.length, icon: PackageCheck, tone: 'bg-emerald-500/10 text-emerald-500' },
    { label: 'Fleet assets', value: vehicles.total, icon: Truck, tone: 'bg-amber-500/10 text-amber-500' },
    { label: 'Paid revenue', value: `EUR ${revenue.toLocaleString()}`, icon: ReceiptText, tone: 'bg-rose-500/10 text-rose-500' },
  ];
  ...
  return <div className="space-y-6">
    <section className="relative overflow-hidden rounded-3xl border border-sky-100 bg-gradient-to-br from-white via-sky-50 to-violet-100 p-6 text-slate-900 shadow-xl shadow-sky-900/10 dark:border-slate-800 dark:from-slate-950 dark:via-slate-950 dark:to-indigo-950 dark:text-white dark:shadow-black/30 md:p-8">
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-6"><div><div className="flex items-center gap-3"><div className="flex h-13 w-13 items-center justify-center rounded-2xl bg-gradient-to-br from-primary to-violet-500 text-white shadow-lg"><Crown className="h-7 w-7" /></div><div><p className="text-xs font-black uppercase tracking-[0.22em] text-primary">Superadmin</p><h1 className="text-3xl font-black">Platform Command Center</h1></div></div><p className="mt-4 max-w-3xl text-sm text-slate-600 dark:text-slate-300">Live visibility across records stored in the Smartfreight database.</p></div><div className="rounded-2xl border border-emerald-300/70 bg-white/70 px-4 py-3 dark:border-emerald-400/20 dark:bg-emerald-400/10"><p className="flex items-center gap-2 text-xs font-black text-emerald-600 dark:text-emerald-400"><ShieldCheck className="h-4 w-4" /> DATABASE CONNECTED</p></div></div>
      <div className="relative mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[['Tracking events', events.total, Activity], ['Database resources', sources.length, ShieldCheck], ['Countries active', countries, Building2], ['Open invoices', invoices.items.filter((invoice) => String(invoice.status).toLowerCase() !== 'paid').length, ReceiptText]].map(([label, value, Icon]) => <div key={String(label)} className="rounded-2xl border border-sky-200/80 bg-white/70 p-4 dark:border-white/10 dark:bg-white/5"><div className="flex items-center gap-2 text-xs uppercase tracking-wider text-slate-500"><Icon className="h-4 w-4 text-primary" />{String(label)}</div><p className="mt-2 text-2xl font-black">{loading ? '—' : String(value)}</p></div>)}</div>
    </section>
    {error ? <Card><InlineDataState loading={false} error={error} empty="" onRetry={() => sources.forEach((source) => void source.refresh())} /></Card> : <>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{metrics.map((metric) => <Card key={metric.label} className="p-4"><div className="flex items-start justify-between"><div><p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">{metric.label}</p><p className="mt-2 text-2xl font-black text-slate-900 dark:text-white">{loading ? '—' : metric.value}</p></div><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${metric.tone}`}><metric.icon className="h-5 w-5" /></div></div></Card>)}</section>
