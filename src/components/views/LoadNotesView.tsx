  ...
  const unifiedRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const searchable = row.kind === 'document'
        ? `${row.document.name} ${row.document.comment} ${documentTypeLabel(lang, row.document.type)} ${loadsById[loadId]?.label || ''}`
