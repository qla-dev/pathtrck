...
export const LoadNotesView = ({ lang }: { lang: Language }) => {
  ...
  const handleCreateNote = async () => {
    if (!draftLoadId || !draftTitle.trim() || !draftText.trim() || !user) return;
    await api.notes.create({ load_id: Number(draftLoadId), author_user_id: user.id, priority: draftPriority.toLowerCase(), body: `${draftTitle.trim()}\n${draftText.trim()}`, is_private: false });
