...
type Member = { id: number; name: string; email: string; role: CompanyRole; status: 'Active' | 'Invited' };

const INITIAL_MEMBERS: Member[] = [
  { id: 1, name: 'John Doe', email: 'john@smartfreight.ai', role: 'Company Admin', status: 'Active' },
  { id: 2, name: 'Lena Kovac', email: 'lena@smartfreight.ai', role: 'Dispatcher', status: 'Active' },
  { id: 3, name: 'Marco Petrovic', email: 'marco@smartfreight.ai', role: 'Driver', status: 'Active' },
  { id: 4, name: 'Sara Lind', email: 'sara@smartfreight.ai', role: 'Finance', status: 'Active' },
];
...
export const CompanyTeamView = ({ lang: _lang }: { lang: Language }) => {
  ...
  const invite = () => {
    ...
    if (members.some((member) => member.email.toLowerCase() === trimmed.toLowerCase())) {
      setMessage('This person is already part of the company.');
      return;
    }
    setMembers((current) => [...current, { id: Date.now(), name: trimmed.split('@')[0], email: trimmed, role, status: 'Invited' }]);
    setEmail('');
            ...
            {members.map((member) => (
              <div key={member.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                ...
                <div className="flex items-center gap-2">
                  <select value={member.role} onChange={(event) => setMembers((current) => current.map((item) => item.id === member.id ? { ...item, role: event.target.value as CompanyRole } : item))} className="h-8 rounded-lg border border-slate-200 bg-white px-2 text-xs font-bold dark:border-slate-700 dark:bg-slate-950 dark:text-white"><option>Driver</option><option>Dispatcher</option><option>Finance</option><option>Company Admin</option></select>
