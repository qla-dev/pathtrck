...
export type SceneView = 'exterior' | 'dimensions' | 'loading' | 'top' | 'side';
type Props = { equipment: Equipment; cargo: Cargo[]; view: SceneView; warehouse?: boolean; selected?: string; onSelect?: (id: string) => void; onMove?: (id: string, x: number, y: number, z: number) => void; reset?: number; zoom?: number; unavailable: string; mini?: boolean };
...
export function PlanningScene(props: Props) {
  ...
  useEffect(() => {
    ...
    const scene = new T.Scene(); scene.background = new T.Color('#edf3f7');
    ...
    const redraw=() => {
      const {equipment:e,cargo,selected,warehouse,mini}=latest.current;
      const oldRoof=roof.position.y; dispose(model);scene.remove(model);model=new T.Group();scene.add(model);
      roof=new T.Group();side=new T.Group();doors=[];cargoMeshes=[];
      ...
      if (!mini) {
        box(model,L/2,-.32,W/2,Math.max(24,L+12),.16,18,'#d9e3e8');
        const grid=new T.GridHelper(30,60,0xbdcdd7,0xd0dce3);grid.position.set(L/2,-.23,W/2);model.add(grid);
        if (warehouse) {
          for (const z of [-5,7]) for (let x=-2;x<L+4;x+=3) {
      ...
      if(latest.current.view==='dimensions'&&!mini) {
    ...
    const changeView=()=>{
      ...
      roofTarget=view==='top'?e.height+2:(view==='dimensions'?e.height+1:0);
      sideTarget=(view==='top'||view==='dimensions'||view==='side')?.12:1;
      ...
      const aspect=Math.max(element.clientWidth,1)/Math.max(element.clientHeight,1),tan=Math.tan(T.MathUtils.degToRad(camera.fov/2)),reach=L/2+(e.truck?2.6:.8);
      ...
      // Bird's-eye stays square to the container: pan and zoom only, no orbiting into a diagonal.
      else{transition=1;controls.enableRotate=view!=='top';controls.target.set(0,view==='top'?0:e.height/2,0);}
      redraw();resize();
    };
    ...
    const down=(event:PointerEvent)=>{
      if(latest.current.mini||event.button!==0||!['top','loading','dimensions','side'].includes(latest.current.view))return;
      ...
      controls.enabled=false;renderer.domElement.setPointerCapture(event.pointerId);event.stopImmediatePropagation();
    };
    ...
    const up=(event:PointerEvent)=>{if(!drag)return;const d=drag;drag=null;controls.enabled=true;if(renderer.domElement.hasPointerCapture(event.pointerId))renderer.domElement.releasePointerCapture(event.pointerId);const {x,z}=d.group.position;d.group.position.copy(d.original);latest.current.onMove?.(d.id,x,d.y,z);};
    const cancel=()=>{if(drag){drag.group.position.copy(drag.original);drag=null;}controls.enabled=!latest.current.mini;};
    ...
    const animate=()=>{
      frame=requestAnimationFrame(animate);const speed=reduced?1:.1;
      if(transition){camera.position.lerp(target,speed);if(camera.position.distanceTo(target)<.01)transition=0;}
    ...
    return()=>{cancelAnimationFrame(frame);observer.disconnect();controls.dispose();dispose(model);renderer.dispose();renderer.domElement.removeEventListener('pointerdown',down,true);renderer.domElement.removeEventListener('pointermove',move);renderer.domElement.removeEventListener('pointerup',up);renderer.domElement.removeEventListener('pointercancel',cancel);renderer.domElement.remove();runtime.current=null;};
  }, []);
  useEffect(()=>{runtime.current?.redraw();},[props.cargo,props.selected,props.warehouse]);
  ...
  return <div ref={host} className="h-full w-full overflow-hidden" role="img" aria-label={`3D ${props.equipment.code} ${props.view}`}>{failed&&<p className="p-6 text-slate-600">{props.unavailable}</p>}</div>;
}
