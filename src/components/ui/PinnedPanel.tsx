 ...
 */
export const PinnedPanel = ({ open, title, subtitle, icon: Icon, onClose, closeLabel, collapseLabel, expandLabel, actions, toolbar, footer, children }: PinnedPanelProps) => {
  ...
  return createPortal(
    <AnimatePresence>
      {open && (
        ...
        >
          <header className="flex h-14 shrink-0 items-center gap-3 border-b border-slate-200 px-3 dark:border-slate-800">
          ...
          {!collapsed && toolbar && <div className="shrink-0 border-b border-slate-200 p-3 dark:border-slate-800">{toolbar}</div>}
          {!collapsed && <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-3">{children}</div>}
          {!collapsed && footer && <footer className="shrink-0 border-t border-slate-200 p-3 dark:border-slate-800">{footer}</footer>}
        </motion.aside>
      )}
    </AnimatePresence>,
    document.body,
